...
Entwickler unterstützen kann. Besonders für größere Programme wird häufig eine Technik angewendet, bei der man
...
In diesem Fall können Mockobjekte gebildet werden, welche es DART erlauben, die Ausführung genauer zu steuern.
Aufbauend darauf kann mit der Generierung des Testdrivers begonnen werden. Dieser enthält alle nötigen Schritte, welche
das Programm testen und Fehler aufdecken \cite{godefroid2005dart}. Im letzten Schritt wird das Programm nun analysiert.
Hierzu wird untersucht, wie sich das Programm verhält, wenn es zufällige Eingabewerte erhält \cite{godefroid2005dart}.
Aufbauend darauf werden nun automatisch neue Testeingabewerte generiert, sodass die Ausführung entlang aller Pfade gelenkt
werden kann \cite{godefroid2005dart}.
...
Ein Problem der Generierung von Testdaten bei DART ist die langsame Ausführung für größere Programme, sodass Godefroid et al.
vorschlagen dies kompositionell durchzuführen. So stellen Godefroid et al. einen neuen Algorithmus vor, welchen sie
\textit{Systematic Modular Automated Random Testing} (SMART) nennen \cite{godefroid2007compositional}.

Dieses Kapitel soll damit fortgesetzt werden, dass es zeigt, wie SMART generell funktioniert. Außerdem werden Vor -und
Nachteile präsentiert und gezeigt, welche Unterschiede zu DART selbst bestehen. Es werden also grundlegende Unterschiede
aufgezeigt und verdeutlicht warum SMART besser geeignet ist als DART.
