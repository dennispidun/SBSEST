...
Dieses Problem wurde in der Vergangenheit damit gelöst, dass es auf das Path Problem reduziert wurde \cite{korel1990dynamic}.
...
Damit Informationen über gewisse Branches zur Verfügung stehen, müssen diese klassifiziert werden. Dazu unterteilt sie
...
Somit ergibt es sich die \textit{critical branches} nicht zu verwerten, da diese nicht zur gewünschten Node Y führen. Interessanter
...
nicht verworfen, da sie keinen Einfluss auf die Node Y haben können \cite{korel1990dynamic}.
...
können nach Tabelle \ref{tab:bspTabelle} in folgende Formen umgewandelt werden: \(F\) \fett{rel} 0.
...
Funktion $F_j(x)$ minimieren und so lange nach einer Lösung suchen, bis $F_j(x)$ negativ wird \cite{korel1990dynamic}.
...
Um die Komplexität des Problems zu verringern, wird zunächst nur ein Teil der Eingabewerte betrachtet. Zeitgleich
bleiben die restlichen Werte konstant. Man sucht also so lange nach einer Lösung für, $x_1$ bis die Branch Funktion
...
Dies sorgt dafür, dass man schneller an das gewünschte Ziel gelangt, sodass man hier keine Ressourcen verschwendet.
...
möglichen Pfade beachtet werden und so mögliche Fehler nicht berücksichtigt werden können \cite{godefroid2005dart} \cite{korel1990dynamic}
...
iN diesem Kapitel soll erforscht werden, ob sich der Ansatz von Korel auch auf \textit{Distributed Software} übertragen
lässt. Zudem sollen Schwierigkeiten bei dieser Art von Software aufgezeigt werden und (eventuelle) Lösungen präsentiert
werden. Der Leser soll somit durch dieses Kapitel ein Verständnis für die Problematik bekommen.
